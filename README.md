...
| `<leader>bg` | Go to a buffer (letter-pick) |
...
| `<leader>wm` | Maximize this window / restore the layout |
...
lazy-lock.json           pinned plugin versions; local only, not committed (.gitignore)
```

`lazy-lock.json` is generated by `:Lazy sync` and gitignored, so each machine
resolves plugin versions independently rather than sharing one pinned set.
`nvim-treesitter` and `nvim-treesitter-textobjects` track `branch = "main"`
(needed for 0.12 compat), so they can drift furthest between machines or over
time — run `:Lazy sync` if something treesitter-related misbehaves after a
long gap.
...
**After changing plugins** — `:Lazy sync`. `lazy-lock.json` isn't committed, so
this only updates the current machine; run it again elsewhere to pick up the
same change.
