...
return {
  ...
  -----------------------------------------------------------------------
  -- GitHub Copilot, as an alternative to Ollama above. Needs a subscription.
  -- Set enabled = true (and minuet to false -- two engines would draw
  -- suggestions over each other), then run :Copilot auth once per machine.
  -----------------------------------------------------------------------
  {
    ...
    opts = {
      suggestion = {
        enabled = true,
        auto_trigger = AUTO_TRIGGER, -- same toggle as the Ollama setup above
        keymap = {
          accept = "<M-y>",
          accept_word = "<M-Y>",
          next = "<M-]>",
      ...
      panel = { enabled = false },
      filetypes = { markdown = true, gitcommit = true, yaml = true, help = false, ["."] = false },
    },
    keys = {
      { "<leader>am", "<cmd>Copilot toggle<cr>", desc = "Toggle inline AI completion" },
      { "<leader>aM", "<cmd>Copilot status<cr>", desc = "Copilot status" },
    },
  },
